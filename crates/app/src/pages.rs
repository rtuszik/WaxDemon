...
fn SyncPanel() -> impl IntoView {
    ...
    }}</p><span class="error">{move ||data.data.get()["run"]["error"].as_str().map(str::to_owned)}</span><div role="status">{move ||data.data.get()["run"]["warnings"].as_array().cloned().unwrap_or_default().into_iter().filter_map(|warning|warning.as_str().map(str::to_owned)).map(|warning|view!{<p>{warning}</p>}).collect_view()}</div><Status remote=data/></div><MutationButton action="/api/sync".into() label="Sync now" refresh=data/></section>}
}
