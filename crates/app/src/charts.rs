...
pub fn history_options(history: &[Value], currency: &str, counts: bool) -> Value {
    let points = history
        .iter()
        .filter(|entry| counts || entry["currency"].as_str().unwrap_or("unknown") == currency)
        .count();
    let series: Vec<_> = if counts {
        let data: Vec<_> = history
            .iter()
            .map(|entry| json!([entry["timestamp"], entry["total_items"]]))
            .collect();
        ...
        .map(|(field, label)| {
            let data: Vec<_> = history
                .iter()
                .filter(|entry| entry["currency"].as_str().unwrap_or("unknown") == currency)
                .map(|entry| {
                    let amount = entry[field]
                        .as_str()
                        .and_then(|value| value.parse::<f64>().ok())
                        .filter(|amount| amount.is_finite());
                    json!([entry["timestamp"], amount])
                })
                .collect();
...
pub fn breakdown_options(values: &[Value]) -> Value {
    let data: Vec<_> = values
        .iter()
        .map(|entry| json!({ "name": entry["name"], "value": entry["count"] }))
        .collect();
    ...
    json!({
        ...
        "color": [
            "#3b82f6", "#a855f7", "#10b981", "#ec4899", "#f97316", "#06b6d4",
            "#f59e0b", "#6366f1", "#84cc16", "#d946ef", "#737373",
        ],
        "tooltip": {
            "trigger": "item",
            "renderMode": "richText",
...
pub fn Chart(
    ...
    view! {
        <div node_ref=node class="chart" role="img" aria-label=label data-chart=label></div>
        {move || {
            error.get().then(|| {
                view! {
                    <p class="error">
